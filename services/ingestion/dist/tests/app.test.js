...
describe('Ingestion Service API Tests', () => {
    ...
    test('rejects an invoice without an ID', async () => {
        const response = await (0, supertest_1.default)(app_1.app).post('/api/v1/expenses').set('Authorization', `Bearer ${mockToken}`).send({ vendor: 'City Cabs', total: 48.0 });
    ...
    test('accepts a unique invoice and publishes invoice.pending', async () => {
        ...
        const response = await (0, supertest_1.default)(app_1.app).post('/api/v1/expenses').set('Authorization', `Bearer ${mockToken}`).send({ id: 'INV-1016', vendor: 'City Cabs', invoiceNumber: 'CC-4410', total: 48.0 });
    ...
    test('returns 404 for an unknown route', async () => {
        const response = await (0, supertest_1.default)(app_1.app).get('/unknown-route');
