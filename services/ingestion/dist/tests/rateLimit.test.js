...
test('handles upstream 429 errors', async () => {
    ...
    const response = await (0, supertest_1.default)(app_1.app).post('/api/v1/expenses').set('Authorization', `Bearer ${header}.${payload}.signature`).send({ id: 'INV-RTLIMIT', vendor: 'Express Courier', invoiceNumber: 'EC-9911', total: 120 });
